...
namespace ReCUDA_Birks3D
{
		       ...
		       std::vector< std::vector<double> > &QY )
  {
    ...
    cudaMalloc( (void**)&DeviceP, sizeof(double) * 96 );
	...
	cudaMemcpy( &QY[fi][ 32 * (ParHead / 96) ], DeviceQY, sizeof(double) * 32, cudaMemcpyDeviceToHost );
	ParHead += 96;
      }
    }
		     ...
		     std::vector< std::vector<double> >  &dEBins )
  {
	...
	cudaMemcpy( &RBins[fi][ 32 * NBINS * (ParHead / 96) ], DeviceRBins, sizeof(double) * 32 * NBINS, cudaMemcpyDeviceToHost );
	cudaMemcpy( &dEBins[fi][ 32 * NBINS * (ParHead / 96) ], DevicedEBins, sizeof(double) * 32 * NBINS, cudaMemcpyDeviceToHost );
	ParHead += 96;
      }
    }

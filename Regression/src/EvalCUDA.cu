...
namespace ReCUDA
{
		     ...
		     std::vector< std::vector<double> >  &dEBins )
  {
	...
	//cudaMemcpy( &dQBins[fi][ParHead / 64], DevicedQBins, sizeof(double) * 32 * NBINS, cudaMemcpyDeviceToHost );
	GetR<<< NBINS, 32 >>>( DevicedQBins, DevicedEBins, DeviceRBins );
	cudaMemcpy( &RBins[fi][ParHead / 64], DeviceRBins, sizeof(double) * 32 * NBINS, cudaMemcpyDeviceToHost );
	cudaMemcpy( &dEBins[fi][ParHead / 64], DevicedEBins, sizeof(double) * 32 * NBINS, cudaMemcpyDeviceToHost );
	ParHead += 64;
      }
    }
